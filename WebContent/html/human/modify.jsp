...
$(document).ready(function() {
    ...
    //just wanna know whether OK if model. dropped
    fillSalary("salaryStandardId","<s:property value="salaryStandardId"/>");
	fillPublicAtt("humanHobby","爱好","<s:property value="humanHobby"/>"); 
	fillPublicAtt("humanNationality","国籍","<s:property value="humanNationality"/>");
	fillPublicAtt("humanReligion","宗教信仰","<s:property value="humanReligion"/>");
	fillPublicAtt("humanParty","政治面貌","<s:property value="humanParty"/>");
	...
	$("#secondKindId").change(function(){
		...
		$.ajax({
			   type: "POST",
			   url: "human/ajax/getThirdClassBySecondClass.action",
			   dataType: 'json',
	...
	function fillMajorClassify(eid){
		$.ajax({
			   type: "POST",
			   url: "human/ajax/getMajorClassify.action",
			   dataType: 'json',
			   data: null,
			   success: function(msg){
				   eid="#"+eid;
	...
	function fillSalary(eid,selected){
		$.ajax({
			   type: "POST",
			   url: "human/ajax/getTblStandardSalaryAll.action",
			   dataType: 'json',
			   data: null,
			   success: function(msg){
				   eid="#"+eid;
	...
	function fillPublicAtt(eid,strType,selected){
		$.ajax({
			   type: "POST",
			   url: "human/ajax/getPublicAttMapByType.action",
			   dataType: 'json',
			   data: "type="+strType,
			   success: function(msg){
				   eid="#"+eid;
	...
	function fillmajor(eid,strClazz,selected){
		$.ajax({
			   type: "POST",
			   url: "human/ajax/getTblMajorNameByClassId.action",
			   dataType: 'json',
			   data: "clazz="+strClazz,
			   success: function(msg){
				   eid="#"+eid;
...
<!-- #actionUrl = /HRM/human/add.action -->
	<form enctype="multipart/form-data" action="<s:property value="#actionUrl"/>" method="post">
		...
		<table class="query_form_table">
			...
			<tr>
				...
				<th>生日</th>
				<td><sd:datetimepicker value="model.humanBirthday" type="date" name="humanBirthday"
						displayFormat="yyyy-MM-dd"></sd:datetimepicker> <!-- <input size="20" /> --></td>
			</tr>
			<tr>
				<th>宗教信仰</th>
			...
			<tr>
				...
				<td>
					<s:if test="#parameters.type[0] == 'check'">
						<sd:datetimepicker value="model.checkTime" type="date" name="checkTime" displayFormat="yyyy-MM-dd"></sd:datetimepicker>
					</s:if> 
					<s:elseif test="#parameters.type[0] == 'update'">
						<sd:datetimepicker value="model.changeTime" type="date" name="changeTime" displayFormat="yyyy-MM-dd"></sd:datetimepicker>
					</s:elseif>
					<s:else>
						<sd:datetimepicker value="model.registTime" type="date" name="registTime" displayFormat="yyyy-MM-dd"></sd:datetimepicker>
					</s:else>
				</td>
				<th>特长</th> 
