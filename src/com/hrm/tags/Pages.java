...
public class Pages extends Component {
	...
	public boolean start(Writer writer) {
		...
		try {
			...
			System.out.println("javax.servlet.forward.request_uri"+url);
			...
			if (theme == null || "text".equals(theme)) {
				...
				if (pageNo.equals(total)) {
					...
					if ("1".equals(total)) {
						...
						// 到达最后一页,显示“[首页] [上一页] [末页]”
						str.append("<a href='" + url + "?pageNo=1" + perUrl
								+ "'>[首页]</a> ");
						str.append("<a href='" + url + "?pageNo="
								+ (cpageInt - 1) + perUrl + "'>[上一页]</a>");
						str.append(" <a href='" + url + "?pageNo=" + total
								+ perUrl + "'>[末页]</a> ");
					}
				} else {
					// 当前页与总页数不相同
				...
				if (totalInt == 1) {
					...
					for (int i = v; i <= v1; i++) {
						if (cpageInt == i) { // 当前页要加粗显示
							...
							str.append("<a href='" + url + "?pageNo=" + i
									+ perUrl + "'>" + i + "</a> ");
						}
					}
					// 如果多于1组并且不是最后一组，显示“> >>”
